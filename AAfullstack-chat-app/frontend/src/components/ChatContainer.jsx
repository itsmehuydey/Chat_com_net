...
const ChatContainer = () => {
    ...
    const renderFile = (file, fileName) => {
        ...
        if (isImage) {
            const imageUrl = file.replace('/auto/upload/', '/image/upload/');
            return (
                <div className="relative">
                    <img
                        src={imageUrl}
                    ...
                    />
                    <p className="text-sm text-red-500 hidden" style={{ display: "none" }}>
                        Không thể tải ảnh
                    </p>
                </div>
            );
        ...
        <div className="flex-1 flex flex-col overflow-auto">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.map((message, index) => (
                    <div
                        key={message._id || index}
                        className={`chat ${message.senderId === authUser._id && !selectedGroup ? "chat-end" : "chat-start"}`}
                        ref={index === messages.length - 1 ? messageEndRef : null}
                    >
                        <div className="chat-image avatar">
                            <div className="size-10 rounded-full border">
                                <img
                                    src={
                                        message.senderId === authUser._id
                                            ? authUser.profilePic || "/avatar.png"
                                            : (selectedUser?.profilePic || message.senderId.profilePic || "/avatar.png")
                                    }
                                    alt="profile pic"
                                />
                            </div>
                        ...
                        <div className="chat-header mb-1">
                            {selectedGroup && (
                                <span className="text-sm">{message.senderId.fullName}</span>
                            )}
                            <time className="text-xs opacity-50 ml-1">
                                {formatMessageTime(message.createdAt)}
                            </time>
